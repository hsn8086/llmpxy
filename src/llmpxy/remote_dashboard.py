        ...
        summary.update(
            "\n".join(
                [
                    ...
                    f"Window(60s): req={window.get('request_count', 0)} rps={self._format_float(window.get('rps'), 2)} tok/s={self._format_float(window.get('tokens_per_second'), 2)} err_rate={self._format_percent(window.get('error_rate'))} avg_latency={self._format_latency(window.get('avg_latency_ms'))}",
                    f"Window(10s): rps={self._window_value(windows, '10s', 'rps')} success_rps={self._window_value(windows, '10s', 'success_rps')} error_rps={self._window_value(windows, '10s', 'error_rps')}",
                    f"Window(5m): req={self._window_value(windows, '5m', 'request_count', integer=True)} p95={self._window_latency(windows, '5m', 'p95_latency_ms')} tok/s={self._window_value(windows, '5m', 'tokens_per_second')} tokens={self._window_value(windows, '5m', 'total_tokens', integer=True)} cost=${self._window_value(windows, '5m', 'total_cost_usd')}",
    ...
    def _render_provider_details(self, provider: dict[str, Any] | None) -> str:
        ...
        return "\n".join(
            [
                ...
                f"Window(60s): req={provider.get('window', {}).get('request_count', 0)} rps={self._format_float(provider.get('window', {}).get('rps'), 2)} tok/s={self._format_float(provider.get('window', {}).get('tokens_per_second'), 2)} err_rate={self._format_percent(provider.get('window', {}).get('error_rate'))}",

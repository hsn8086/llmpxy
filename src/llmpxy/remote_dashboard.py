        ...
        summary.update(
            "\n".join(
                [
                    ...
                    f"Window(60s): req={window.get('request_count', 0)} rps={self._format_float(window.get('rps'), 2)} tok/s={self._format_float(window.get('tokens_per_second'), 2)} cache_hit={self._format_percent(window.get('cache_hit_rate'))} err_rate={self._format_percent(window.get('error_rate'))} avg_latency={self._format_latency(window.get('avg_latency_ms'))}",
